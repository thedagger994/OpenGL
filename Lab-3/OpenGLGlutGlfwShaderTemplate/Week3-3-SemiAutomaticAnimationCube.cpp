...
void display(void)
{
	...
	//counter += 0.1f;
	transformObject(0.25f, YZ_AXIS, counter++, glm::vec3(0.5f, 0.5f, 0.0f));
	//Ordering the GPU to start the pipeline
	glDrawElements(GL_QUADS, 24, GL_UNSIGNED_SHORT, 0);

	transformObject(0.25f, XZ_AXIS, counter++, glm::vec3(-0.5f, 0.5f, 0.0f));
	glDrawElements(GL_QUADS, 24, GL_UNSIGNED_SHORT, 0);

	transformObject(0.25f, Y_AXIS, counter++, glm::vec3(0.5f, -0.5f, 0.0f));
	glDrawElements(GL_QUADS, 24, GL_UNSIGNED_SHORT, 0);

	transformObject(0.25f, Z_AXIS, counter++, glm::vec3(-0.5f, -0.5f, 0.0f));
	glDrawElements(GL_QUADS, 24, GL_UNSIGNED_SHORT, 0);
...
void timer(int)
{
	...
	glutTimerFunc(1000 / 60, timer, 0); //in one second, this function calls itself 60 times
}
